...
2. auf der Maschine muss der Nutzer `user` mit dem Kennwort `hallowelt` vorhanden sein
    * Gruppe `sudo` + `ssh`
    * diesen Nutzer benötigt ansible um sich beim ersten mal auf diesen PC zu verbinden
    * falls bereits cloudinit die vm erstellt hat sollte der Nutzer bereits vorhanden sein
3. Playbook bootstrap auf DIESEM Rechner ausführen (limit beachten)
